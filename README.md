...
To install the packaged CLI after building it locally, run `npm install -g ./polyglot-agent-harness-0.1.0.tgz`, then invoke `agent run "your task" --workspace /path/to/repo`. The package requires Node.js 20 or newer.
...
The CLI is review-only by default. `accept-edits` permits file changes but requests shell/Git actions; `auto` permits only the built-in safe test/build allowlist. Other commands are always treated as approval-required. In an interactive terminal, required approvals get an explicit prompt; JSON/headless runs deny them. Each run writes JSONL events to `.agent/sessions/`.
...
Before a coding run, declared test/build/lint commands are snapshotted from the repository. The standard `run_test` and `run_build` tools discover Node package scripts plus Maven, Gradle, and Python (`pyproject.toml`/`pytest.ini`) projects; Python test runs prefer `uv run pytest` when `uv.lock` exists. After an edit, the runtime executes that snapshot through the policy gate and returns failed output to the model for repair. Newly written package scripts are never trusted as verification commands during the same run.

Inspect a completed session with `pnpm agent -- session SESSION_ID --workspace /path/to/repo --json`, or continue it with `pnpm agent -- resume SESSION_ID "follow-up task" --workspace /path/to/repo`. Press `Ctrl-C` to stop between model/tool operations.

See [architecture](docs/architecture/architecture-decision-record.md) and [research notes](docs/research/agent-harnesses.md).

Copy `.agent/config.example.json` to `.agent/config.json` to configure a repository. Keep API keys in the named environment variable, not in the configuration file.
...
Use `POST /runs/background` to start a managed asynchronous run; it returns a `sessionId`, and `GET /runs/{sessionId}` returns its current state, result, and emitted events. `DELETE /runs/{sessionId}` requests cancellation for a running background task. All server and ACP run requests accept an optional `sessionId` and `resume: true` to continue a persisted session.

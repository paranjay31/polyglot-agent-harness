...
import { loadRepositoryConfig, type AgentConfig } from "../../../packages/config/src/index.js";
...
import { PatternPolicyEngine, type Permission, type PolicyDecision, type PolicyRequest } from "../../../packages/policy/src/index.js";
...
import { JsonlSessionStore } from "../../../packages/session/src/index.js";
...
async function registerConfiguredMcp(config: AgentConfig, registry: ToolRegistry): Promise<McpClient[]> { const clients: McpClient[] = []; for (const server of config.mcp) { const headers = { ...server.headers, ...(server.bearerTokenEnv && process.env[server.bearerTokenEnv] ? { authorization: `Bearer ${process.env[server.bearerTokenEnv]}` } : {}) }; const client = server.transport === "stdio" ? await StdioMcpClient.connect(server.id, server.command ?? (() => { throw new Error(`MCP ${server.id} requires command`); })(), server.args ?? []) : new HttpMcpClient(server.id, server.endpoint ?? (() => { throw new Error(`MCP ${server.id} requires endpoint`); })(), headers); await new McpToolAdapter(client).register(registry); clients.push(client); } return clients; }
...
async function main() {
  ...
  const registry = new ToolRegistry(); registerBuiltinTools(registry); const clients = await registerConfiguredMcp(config, registry); const memory = new InMemoryEventSink(), metrics = new MetricsEventSink(); const events = new RedactingEventSink(new FanoutEvents([memory, store, metrics, new TerminalEventSink()]));
  ...
  const result = await runtime.run({ id: "cli", prompt: task, workspace }, { maxTurns: Number.isFinite(maxTurns) && maxTurns > 0 ? maxTurns : 12, signal: controller.signal, sessionId: resumeSessionId }); await Promise.all([...clients.map(client => client.close?.()), languageServer?.shutdown()]);

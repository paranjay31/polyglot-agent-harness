...
async function main() {
  ...
  const runtime = new AgentRuntime(selected, registry, new ProgressiveContextEngine(repository), events, toolContext, new ToolVerificationRunner(() => new RepositoryVerificationPlanner(workspace).commands(), registry, toolContext), new SkillRegistry(workspace, config.skills.enabled));

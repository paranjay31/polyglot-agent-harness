...
export interface ToolContext { workspace: WorkspaceAccess; sandbox: Sandbox; policy: PolicyEngine; knowledge?: RepositoryKnowledgeProvider; languageServer?: LanguageServer; approve?: (request: PolicyRequest & { tool: string; input: unknown }) => Promise<boolean> }
export interface ToolResult { content: string; data?: unknown; execution?: ExecutionResult }
export interface Tool<I = unknown> { name: string; description: string; inputSchema: ZodType<I>; modelSchema?: object; permission: Permission; risk?: CommandRisk; execute(input: I, context: ToolContext): Promise<ToolResult> }
...
export class ToolRegistry {
  ...
  async invoke(name: string, rawInput: unknown, context: ToolContext): Promise<ToolResult> { const tool = this.tools.get(name); if (!tool) throw new Error(`Unknown tool: ${name}`); const input = tool.inputSchema.parse(rawInput); const command = (input as { command?: Command }).command; const request = { permission: tool.permission, risk: name === "shell" && command ? classifyCommand(command) : tool.risk, target: command ? command.argv.join(" ") : name }; const decision = context.policy.decide(request); if (decision === "deny") throw new Error(`Policy deny: ${tool.permission} for ${name}`); if (decision === "ask" && !(await context.approve?.({ ...request, tool: name, input }))) throw new Error(`Policy ask: ${tool.permission} for ${name}`); return tool.execute(input, context); }
}
export function registerBuiltinTools(registry: ToolRegistry) {
  ...
  const declaredScript = async (ctx: ToolContext, script: "test" | "build" | "lint" | "format") => {
    const file = async (name: string) => ctx.sandbox.readFile(join(ctx.workspace.root, name)).catch(() => undefined);

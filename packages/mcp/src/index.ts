...
export class StdioMcpClient implements McpClient {
  ...
  constructor(readonly id: string, command: string, args: string[] = [], private readonly timeoutMs = 30_000) { this.process = spawn(command, args, { stdio: "pipe" }); this.process.stdout.setEncoding("utf8"); this.process.stdout.on("data", chunk => this.onData(chunk)); this.process.on("error", error => this.failAll(error)); this.process.on("exit", () => this.failAll(new Error(`MCP server ${id} exited`))); }
  static async connect(id: string, command: string, args: string[] = []) { const client = new StdioMcpClient(id, command, args); await client.request("initialize", { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "polyglot-agent-harness", version: "0.1.0" } }); client.notify("notifications/initialized", {}); return client; }
  ...
  private request(method: string, params: unknown): Promise<any> { const id = this.nextId++; const payload = JSON.stringify({ jsonrpc: "2.0", id, method, params }); this.process.stdin.write(`Content-Length: ${Buffer.byteLength(payload)}\r\n\r\n${payload}`); return new Promise((resolve, reject) => { const timer = setTimeout(() => { this.pending.delete(id); reject(new Error(`MCP ${method} timed out`)); }, this.timeoutMs); this.pending.set(id, { resolve, reject, timer }); }); }
  ...
  async callTool(name: string, input: unknown): Promise<ToolResult> { const result = await this.request("tools/call", { name, arguments: input }); const content = Array.isArray(result.content) ? result.content.map((item: any) => item.text ?? JSON.stringify(item)).join("\n") : JSON.stringify(result); return { content, data: result }; }
...
export class HttpMcpClient implements McpClient {
  ...
  private async request(method: string, params: unknown) { const response = await fetch(this.endpoint, { method: "POST", headers: { "content-type": "application/json", ...this.headers }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method, params }) }); if (!response.ok) throw new Error(`MCP HTTP request failed (${response.status})`); const body = await response.json() as any; if (body.error) throw new Error(body.error.message ?? "MCP error"); return body.result; }
  async listTools() { return (await this.request("tools/list", {})).tools as McpToolDefinition[]; }
  async callTool(name: string, input: unknown): Promise<ToolResult> { const result = await this.request("tools/call", { name, arguments: input }); return { content: Array.isArray(result.content) ? result.content.map((item: any) => item.text ?? JSON.stringify(item)).join("\n") : JSON.stringify(result), data: result }; }
}
